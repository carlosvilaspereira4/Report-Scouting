...
export function ReportForm({ formData, onChange, onSubmit, errors }: Props) {
  ...
  // Auto-create player on first render if null
  if (!formData.player) {
    const newPlayer = { ...EMPTY_PLAYER, id: crypto.randomUUID() };
    setTimeout(() => update('player', newPlayer), 0);
  }

  const player = formData.player ?? { ...EMPTY_PLAYER, id: 'temp' };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <PlayerInfoDisplay
        player={player}
        onUpdate={(updatedPlayer: Player) => update('player', updatedPlayer)}
      />
      {showErrors && errors?.player && (
        <p className="text-xs text-red-500">{errors.player}</p>
      )}
      ...
      <section>
        ...
        <input
          ...
          placeholder="Ex: Carlos Vilas Pereira"
          className="w-full rounded-lg border border-gray-300 bg-white px-4 py-3 text-sm focus:border-brand-500 focus:ring-2 focus:ring-brand-200 focus:outline-none"
        />
